...
def initialize_sentiment_pipeline():
    """
    Load HuggingFace sentiment pipeline once at startup.
    Env vars for TF/transformers must be set in app.py before imports.
    """
    try:
        os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "3")
        from transformers import pipeline

        print(f"[SENTIMENT] Loading model: {SENTIMENT_PRIMARY_MODEL}")
        ai_config.sentiment_model = pipeline(
            "sentiment-analysis",
            model=SENTIMENT_PRIMARY_MODEL,
            tokenizer=SENTIMENT_PRIMARY_MODEL,
        )
        ai_config.sentiment_available = True
        ai_config.sentiment_error = None
        print("[SENTIMENT] Transformers pipeline ready")
    except Exception as e:
        print(f"Error details: {e}")
        ai_config.sentiment_available = False
        ai_config.sentiment_model = None
        ai_config.sentiment_error = str(e)
        print("[SENTIMENT] Using rule-based fallback only")
...
def analyze_sentiment(text):
    ...
    # Try ML model first (read from ai_config so startup init applies)
    if ai_config.sentiment_model is not None:
        try:
            result = ai_config.sentiment_model(text_input)[0]
...
def is_sentiment_available():
    return ai_config.sentiment_available
...
def analyze_text_sentiment(text):
    ...
    model_used = SENTIMENT_PRIMARY_MODEL if ai_config.sentiment_available else "rule_based_fallback"
