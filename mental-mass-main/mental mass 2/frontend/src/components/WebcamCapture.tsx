...
import {
  normalizeFaceAnalysisPayload,
  type NormalizedFaceResult,
} from "@/services/mentalMassApi";

type FaceResult = NormalizedFaceResult & { method?: string };
...
const WebcamCapture = ({
  ...
  useEffect(() => {
    ...
    const onEmotion = (payload: unknown) => {
      const n = normalizeFaceAnalysisPayload(payload);
      if (n.emotion) {
        setStreamingResult(n);
        onResultRef.current?.(n);
      }
    };
    socket.on("emotion_detected", onEmotion);

    return () => {
      socket.off("emotion_detected", onEmotion);
      socket.close();
  ...
  const analyzeWebcamFrame = async () => {
    ...
    setError(null);
    try {
      const raw = await analyzeFaceImage(frameBase64);
      const normalized = normalizeFaceAnalysisPayload(raw);
      setStreamingResult(normalized);
      setError(null);
      onResult?.(normalized);
    } catch (error: any) {
  ...
  const analyzeUploaded = async () => {
    ...
    try {
      ...
      const raw = await analyzeFace(formData);
      const normalized = normalizeFaceAnalysisPayload(raw);
