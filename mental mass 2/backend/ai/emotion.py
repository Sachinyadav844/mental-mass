...
def _analyze_image_for_emotion(img, num_iterations=3, enforce_detection=True):
    try:
        ...
        # ENSEMBLE APPROACH: DeepFace + FER
        results = []

        # Run DeepFace ensemble (3 iterations)
        if DEEPFACE_AVAILABLE:
            deepface_result = _run_deepface_ensemble(face_img, num_iterations, enforce_detection=enforce_detection)
            if deepface_result and deepface_result['confidence'] >= 0.6:
                results.append(deepface_result)

        # Run FER fallback
        fer_result = _run_fer_fallback(face_img)
        if fer_result and fer_result.get('confidence', 0) >= 0.5:
            results.append(fer_result)

        if not results:
            print('[EMOTION] All models failed or low confidence')
            return _uncertain_emotion()

        # Majority voting across all results
